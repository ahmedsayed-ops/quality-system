...
// ── Normalize mixed answers for analytics only ───────────────────
// q3 can be Yes/No while the rest are Aware/Not Aware.
// Dashboard analytics utilities expect Aware / Not Aware.
// So we normalize only for calculations, while keeping raw records
// intact for table/export display.
function normalizeAnswer(value) {
  if (value === "Yes") return "Aware";
  if (value === "No") return "Not Aware";
  return value;
}

function normalizeRecordForAnalytics(record) {
  return {
    ...record,
    q1: normalizeAnswer(record.q1),
    q2: normalizeAnswer(record.q2),
    q3: normalizeAnswer(record.q3),
    q4: normalizeAnswer(record.q4)
  };
}
...
export default function DashboardPage() {
  ...
  // ── Filtered raw records (for table/export) ────────────────────
  const records = useMemo(() => filterRecords(allRecords, filters), [allRecords, filters]);

  // ── Normalized records (for analytics/KPIs/charts only) ───────
  const analyticsRecords = useMemo(
    () => records.map(normalizeRecordForAnalytics),
    [records]
  );

  const agents = useMemo(
    () => [...new Set(allRecords.map((r) => r.agentUser).filter(Boolean))].sort(),
    [allRecords]
  );

  const branches = useMemo(
    () => [...new Set(allRecords.map((r) => r.branchUser).filter(Boolean))].sort(),
    [allRecords]
  );

  const kpis = useMemo(() => computeKPIs(analyticsRecords), [analyticsRecords]);
  const questionData = useMemo(
    () => computeQuestionBreakdown(analyticsRecords),
    [analyticsRecords]
  );
  const agentRanking = useMemo(
    () => computeAgentRanking(analyticsRecords),
    [analyticsRecords]
  );
  const branchRanking = useMemo(
    () => computeBranchRanking(analyticsRecords),
    [analyticsRecords]
  );
  const dailyTrend = useMemo(
    () => computeDailyTrend(analyticsRecords),
    [analyticsRecords]
  );
  ...
  const pieData = useMemo(
    () => [
      {
        name: "Positive",
        value: kpis.totalAware,
        pct: kpis.awarePercent
      },
      {
        name: "Negative",
        value: kpis.totalNotAware,
        pct: kpis.notAwarePercent
      }
    ],
    [kpis]
  );
    ...
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-5 animate-fade-in">
        <div className="relative w-16 h-16">
          ...
          <div
            className="absolute inset-2 rounded-full border-2 border-brand-300 border-b-transparent animate-spin"
    ...
    <div className="max-w-7xl mx-auto px-4 py-6 sm:py-8 animate-fade-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4 mb-6">
        {loading ? (
          ...
          <>
            ...
            <KpiCard
              title="Positive"
              value={kpis.totalAware.toLocaleString()}
              subtitle="Aware + Yes responses"
              icon="✅"
              color="green"
              barPercent={kpis.awarePercent}
            />
            <KpiCard
              title="Negative"
              value={kpis.totalNotAware.toLocaleString()}
              subtitle="Not Aware + No responses"
              icon="❌"
              color="red"
              barPercent={kpis.notAwarePercent}
            />
            <KpiCard
              title="Positive %"
              value={`${kpis.awarePercent}%`}
            ...
            <KpiCard
              title="Negative %"
              value={`${kpis.notAwarePercent}%`}
              subtitle="overall negative rate"
              icon="📉"
              color="amber"
              barPercent={kpis.notAwarePercent}
            />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-5 mb-5">
        ...
        <div className="chart-card">
          <div className="mb-5">
            <h3 className="font-display font-bold text-slate-800 text-sm">Overall Response Split</h3>
            <p className="text-slate-400 text-xs mt-0.5">
              Positive vs negative responses across all visits and questions
            </p>
          </div>
            ...
            <ResponsiveContainer width="100%" height={280}>
              <PieChart>
                <Pie
                  ...
                  labelLine={false}
                >
                  <Cell fill={POSITIVE_COLOR} stroke="none" />
                  <Cell fill={NEGATIVE_COLOR} stroke="none" />
                </Pie>

                <Tooltip
                  contentStyle={TOOLTIP_STYLE}
                  formatter={(val, name, props) => [
                    `${val} answers (${props.payload.pct}%)`,
                    name
                  ]}
                />
        ...
        <div className="chart-card">
          <div className="mb-5">
            <h3 className="font-display font-bold text-slate-800 text-sm">Positive % per Question</h3>
            <p className="text-slate-400 text-xs mt-0.5">
              Aware/Yes responses by topic
            </p>
          </div>
            ...
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={questionData} margin={{ top: 20, right: 16, left: -8, bottom: 0 }}>
                ...
                <Tooltip
                  contentStyle={TOOLTIP_STYLE}
                  formatter={(v, _, p) => [
                    `${v}% (${p.payload.aware} positive / ${p.payload.notAware} negative)`,
                    "Response Rate"
                  ]}
      ...
      <div className="chart-card mb-5">
        <div className="mb-5">
          <h3 className="font-display font-bold text-slate-800 text-sm">Daily Response Trend</h3>
          <p className="text-slate-400 text-xs mt-0.5">
            Positive vs negative answers over time
          </p>
        </div>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={dailyTrend} margin={{ top: 8, right: 16, left: -8, bottom: 0 }}>
              ...
              <Line
                ...
                strokeWidth={2.5}
                dot={{ r: 4, fill: NEGATIVE_COLOR, strokeWidth: 0 }}
                activeDot={{ r: 6 }}
              />
            </LineChart>
          </ResponsiveContainer>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-5 mb-6">
        <RankingCard
          title="Field Agent Ranking"
          subtitle="Ranked by positive response rate"
          icon="🏆"
          data={agentRanking}
          nameCol="Agent"
        />
        <RankingCard
          ...
          subtitle="Ranked by overall positive score"
          icon="🏪"
          data={branchRanking}
          nameCol="Branch"
        />
...
function RankingCard({ title, subtitle, icon, data, nameCol }) {
  ...
  return (
    <div className="chart-card">
      ...
      {data.length === 0 ? (
        <EmptyChart message={`No ${nameCol.toLowerCase()} data yet`} />
      ) : (
        <>
          <ResponsiveContainer width="100%" height={chartHeight}>
            <BarChart layout="vertical" data={top8} margin={{ top: 0, right: 48, left: 8, bottom: 0 }}>
              ...
              <Tooltip
                contentStyle={TOOLTIP_STYLE}
                formatter={(v, _, p) => [
                  `${v}% positive (${p.payload.visits} visit${p.payload.visits !== 1 ? "s" : ""})`,
                  nameCol
                ]}
              />
              <Bar dataKey="awarePercent" radius={[0, 6, 6, 0]} maxBarSize={24}>
                {top8.map((e, i) => (
                  <Cell key={i} fill={perfColor(e.awarePercent)} />
                ))}
                <LabelList
                  dataKey="awarePercent"
                  position="right"
          ...
          <div className="mt-4 overflow-x-auto">
            <table className="w-full text-xs min-w-[280px]">
              <thead>
                <tr className="border-b border-slate-100">
                  {["#", nameCol, "Visits", "Positive", "%"].map((h) => (
                    <th
                      key={h}
                      className={`py-2 px-2 font-display font-semibold uppercase tracking-wide text-slate-400 ${
              ...
              <tbody>
                {data.slice(0, 6).map((row, i) => (
                  <tr key={i} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    ...
                    <td
                      className="py-1.5 px-2 font-semibold text-slate-700 max-w-[100px] truncate"
                      title={row.name}
                    >
                      {row.name}
                    </td>
            ...
            {data.length > 6 && (
              <p className="text-xs text-slate-400 text-center py-2">+{data.length - 6} more</p>
            )}
          </div>
        </>
      )}
