...
export default function FormPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Build the exact payload the Apps Script expects — no wrapper object
      await submitVisit({
    ...
    return (
      <div className="max-w-md mx-auto px-4 py-16 flex flex-col items-center text-center animate-fade-in">
        <div className="relative mb-8">
          ...
          <div className="relative w-24 h-24 rounded-full bg-emerald-50 border-4 border-emerald-200
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 sm:py-8 animate-fade-in">
      ...
      <div className="flex items-center gap-3 mb-6 p-3.5 bg-brand-50 border border-brand-100
      ...
      <FormSection title="Visit Information" icon="👤" number={1}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <FormField id="branchUser" label="Branch User" error={errors.branchUser} required>
            <input id="field-branchUser" autoComplete="off"
          ...
          <FormField id="agentUser" label="Field Agent User" error={errors.agentUser} required>
            <input id="field-agentUser" autoComplete="off"
          ...
          <FormField id="employeeName" label="Branch Employee Name" error={errors.employeeName} required>
            <input id="field-employeeName" autoComplete="off"
          ...
          <FormField id="mobile" label="Mobile Number" hint="Numbers only" error={errors.mobile} required>
            <input id="field-mobile" inputMode="numeric" type="tel" maxLength={15}
      ...
      <FormSection title="Quality Assessment" icon="✅" number={2}>
        <p className="text-slate-400 text-xs mb-4">
          For each question, select whether the branch is{" "}
          <strong className="text-emerald-600">Aware</strong> or{" "}
      ...
      <FormSection title="Comments & Notes" icon="💬" number={3}>
        <FormField id="comments" label="Observations / Follow-up Actions" error={errors.comments} required>
          <textarea id="field-comments" rows={4}
      ...
      <p className="text-center text-xs text-slate-400 mt-3">
        All fields marked <span className="text-red-400 font-semibold">*</span> are required.
        Data is saved securely to Google Sheets.
      </p>
    </div>
  );
}
...
function FormField({ id, label, children, error, hint, required }) {
  return (
    <div>
      <label htmlFor={id}
        className="block text-xs font-display font-semibold text-slate-600 mb-1.5">
        {label}
...
function AwarenessQuestion({ qKey, index, label, value, error, onChange }) {
  return (
    <div id={`field-${qKey}`}
      ...
      <div className={`text-sm font-display font-semibold mb-3 flex items-start gap-2
      ...
      <div className="flex gap-3">
        {ANSWER_OPTIONS.map(opt => {
          ...
          const isYes    = opt.value === "Aware";
          return (
            <button key={opt.value} type="button"
              onClick={() => onChange(opt.value)}
              aria-pressed={selected}
              className={`aware-btn ${selected ? (isYes ? "aware-sel-yes" : "aware-sel-no") : "aware-unsel"}`}>
